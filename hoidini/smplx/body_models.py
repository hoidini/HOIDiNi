...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        **kwargs,
        ...
        vertices, joints, global_joint_transforms = (
            lbs(  # <---- global_joint_transforms is our addition (Roey)
                shape_components,
                full_pose,
                self.v_template,
                shapedirs,
                ...
                self.lbs_weights,
                pose2rot=pose2rot,
                return_transforms=True,  # <---- our addition (Roey)
            )
        )
        ...
        if self.joint_mapper is not None:
            raise NotImplementedError(
                "Should be applied to the global_joints_transforms as well"
            )
        ...
        if apply_trans:
            joints += transl.unsqueeze(dim=1)
            vertices += transl.unsqueeze(dim=1)
            global_joint_transforms[:, :, :3, 3] += transl.unsqueeze(
                dim=1
            )  # <---- our addition (Roey)

        assert torch.allclose(
            global_joint_transforms[:, :, :3, 3], joints[:, :55]
        )  # <---- our addition (Roey)
        ...
        output = SMPLXOutput(
            ...
            v_shaped=v_shaped,
            full_pose=full_pose if return_full_pose else None,
            global_joints_transforms=global_joint_transforms,
        )
        return output
