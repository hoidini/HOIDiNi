...
def lbs(
    ...
    J_transformed, A, transforms = batch_rigid_transform(
    ...
    if return_transforms:
        return verts, J_transformed, transforms  # <---- our addition (Roey)
    else:
...
def batch_rigid_transform(
    ...
    # rel_joints = torch.cat([joints[:, :1], joints[:, 1:] - joints[:, parents[1:]]], dim=1)
    ...
    # res_matmul = torch.matmul(transforms, joints_homogen)
    # res_padded = F.pad(res_matmul, [3, 0, 0, 0, 0, 0, 0, 0])
    # rel_transforms = transforms - res_padded
    ...
    return (
        ...
        rel_transforms,
        transforms.clone(),
    )  # <---- transforms is our addition (Roey)
