...
def smpldata_to_smplrifkefeats(smpldata, n_joints=52) -> Tensor:
...
def smplrifkefeats_to_smpldata(features: Tensor, n_pose_joints: int):
    (
        root_grav_axis,        # (543,)
        vel_trajectory_local,  # (543, 2)
        vel_angles,            # (543,)
        poses_local,           # (543, 52, 6)
        joints_local,          # (543, 51, 3)
    ) = ungroup(features, n_pose_joints)
...
def group(root_grav_axis, vel_trajectory_local, vel_angles, poses_local, joints_local):
    ...
    # assert features.shape[-1] == 205, features.shape[-1]
    return features


def ungroup(features: Tensor, n_pose_joints: int) -> tuple[Tensor]:
    ...
    (
        ...
        poses_local_flatten,
        joints_local_flatten,
    ) = einops.unpack(features, [[], [2], [], [n_pose_joints * 6], [(n_pose_joints - 1) * 3]], "k *")
