...
from hoidini.closd.diffusion_planner.utils.misc import WeightedSum
from hoidini.object_conditioning.object_encoder_global import ObjectEncoder

normal_repr = torch.Tensor.__repr__
torch.Tensor.__repr__ = (
    lambda self: f"{list(self.shape)}_{normal_repr(self)}"
)  # for debug
...
class MDM(nn.Module):
    def __init__(
        ...
        **kargs,
        ...
        assert self.emb_before_mask is False, "Deleted for simplicity"
        self.mask_frames = kargs.get("mask_frames", False)
        self.arch = arch
        # self.gru_emb_dim = self.latent_dim if self.arch == 'gru' else 0
        # self.input_process = InputProcess(self.data_rep, self.input_feats+self.gru_emb_dim, self.latent_dim)
        #                                   hml_vec                     556 (e.g. 263) + 0 or 1                    512
        self.input_process = InputProcess(
            self.data_rep,
            self.input_feats + (self.keyframe_cond_type != ""),
            self.latent_dim,
        )
        #            <--- in practice, a linear layer from 566 + 0 or 1 to 512
        ...
        if kargs.get("use_obj_encoder", False):
            self.object_encoder = ObjectEncoder(out_dim=self.latent_dim)
        if kargs.get("use_bps_mlp", False):
            # self.bps_mlp = BpsMlp(in_dim=3, out_dim=self.latent_dim, hidden_dim=self.latent_dim, hidden_layers=3)
            raise NotImplementedError

    def encode_obj(self, obj_points):
        # obj_points - [bs, n_points, 3]
        obj_emb = self.object_encoder(obj_points)  # [bs, d]
        obj_emb = obj_emb.unsqueeze(0)  # [1, bs, d]
        return obj_emb
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.keyframe_cond_type != "":
            ...
            x = (y["clean_motion"] * y["keyframe_mask"]) + (
                x * ~y["keyframe_mask"]
            )  # [batch_size, njoints, nfeats, max_frames]
            keyframes_ch = y["keyframe_mask"].to(x.dtype) * 2.0 - 1.0
            x = torch.cat(
                [x, keyframes_ch], dim=1
            )  # [batch_size, njoints+1, nfeats, max_frames]
        ...
        if "text" in self.cond_mode:
            ...
            if "obj_emb" in y.keys():  # caching option
                obj_emb = y["obj_emb"]
            elif "obj_points" in y.keys():
                obj_emb = self.encode_obj(y["obj_points"])
            else:
                obj_emb = None
            ...
            if self.emb_policy == "add":
                ...
                if obj_emb is not None:
                    emd_lst = [time_emb, obj_emb, text_emb]
                    n_non_text_tokens = 2
                else:
                    emd_lst = [time_emb, text_emb]
                    n_non_text_tokens = 1
                emb = torch.cat(
                    emd_lst, dim=0
                )  # [time_emb, w0, w1, ..., wn, empty, empty, ...]
                text_mask = torch.cat(
                    [torch.zeros_like(text_mask[:, 0:n_non_text_tokens]), text_mask],
                    dim=1,
                )  # [    F,    T,  T,  ...,  T,   F,     F,   ...]
        if "action" in self.cond_mode:
...
class EmbedTargetLocSingle(nn.Module):
    def __init__(self, all_goal_joint_names, latent_dim, num_layers=1):
        ...
        self.target_cond_dim = (
            len(self.extended_goal_joint_names) * 4
        )  # 4 => (x,y,z,is_valid)
        self.latent_dim = latent_dim
    ...
    def forward(self, input, target_joint_names, target_heading):
        ...
        for sample_idx, sample_joint_names in enumerate(target_joint_names):
            sample_joint_names_w_heading = (
                np.append(sample_joint_names, "heading")
                if target_heading[sample_idx]
                else sample_joint_names
            )
            for j in sample_joint_names_w_heading:
                validity[sample_idx, self.extended_goal_joint_names.index(j)] = 1.0
...
class EmbedTargetLocSplit(nn.Module):
    ...
    def forward(self, input, target_joint_names, target_heading):
        ...
        mlp_splits = [
            self.mini_mlps[i](mlp_input[:, i]) for i in range(mlp_input.shape[1])
        ]
...
class EmbedTargetLocMulti(nn.Module):
    def __init__(self, all_goal_joint_names, latent_dim):
        ...
        self.extended_goal_joint_idx = {
            joint_name: idx
            for idx, joint_name in enumerate(self.extended_goal_joint_names)
        }
        self.n_extended_goal_joints = len(self.extended_goal_joint_names)
        self.target_loc_emb = nn.ParameterDict(
            {
                joint_name: nn.Sequential(
                    nn.Linear(3, latent_dim),
                    nn.SiLU(),
                    nn.Linear(latent_dim, latent_dim),
                )
                for joint_name in self.extended_goal_joint_names
            }
        )  # todo: check if 3 works for heading and traj
    ...
    def forward(self, input, target_joint_names, target_heading):
        ...
        for sample_idx, sample_joint_names in enumerate(target_joint_names):
            ...
            output_one_sample = torch.zeros(
                (self.n_extended_goal_joints, self.latent_dim),
                dtype=input.dtype,
                device=input.device,
            )
