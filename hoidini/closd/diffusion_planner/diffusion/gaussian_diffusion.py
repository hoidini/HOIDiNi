...
from hoidini.closd.diffusion_planner.diffusion.nn import mean_flat, sum_flat
from hoidini.closd.diffusion_planner.diffusion.losses import (
    normal_kl,
    discretized_gaussian_log_likelihood,
)
from hoidini.closd.diffusion_planner.data_loaders.humanml.scripts import motion_process
from hoidini.closd.diffusion_planner.utils.loss_util import masked_l2, masked_goal_l2
...
class GradEnable:
    ...
    def __enter__(self):
        torch.set_grad_enabled(self.enabled)
...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, t, noise=None, model_kwargs=None):
        ...
        if model_kwargs is not None and "condition_mask" in model_kwargs["y"].keys():
            noise *= ~model_kwargs["y"]["condition_mask"]
        return (
            _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start
    ...
    def p_mean_variance(
        ...
        if recon_guidance and t[0] in model_kwargs["y"]["recon_steps"]:
            ...
            ), "This feature supports only X_start pred for now!"
            ...
            with torch.enable_grad():
                ...
                # t2 = time.time()
                cond_grad = torch.autograd.grad(guidance_loss, z)[
                    0
        ...
        if (
            ...
            "condition_mask" in model_kwargs["y"].keys()
            and "condition_input" in model_kwargs["y"].keys()
        ):
            inpainting_mask, inpainted_motion = (
                model_kwargs["y"]["condition_mask"],
                model_kwargs["y"]["condition_input"],
            )
            assert (
                self.model_mean_type == ModelMeanType.START_X
            ), "This feature supports only X_start pred for mow!"
    ...
    def p_sample(
        ...
        # This was part of the original code and was commented out to prevent a conflict with recon_guidance: GUY 22/5/24
    ...
    def p_sample_with_grad(
        ...
        denoised_fn=None,
        cond_fn=None,
        recon_guidance=False,  # placeholder, not in use
        model_kwargs=None,
        const_noise=False,
    ):
        """
        ...
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :param const_noise: If True, will noise all samples with the same noise.
        :return: a dict containing the following keys:
                 - 'sample': a random sample from the model.
                 - 'pred_xstart': a prediction of x_0.
        """
        with th.enable_grad():
            ...
            if (
                ...
                and "condition_mask" in model_kwargs["y"].keys()
            ):
                noise *= ~model_kwargs["y"]["condition_mask"]
            nonzero_mask = (
                (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
    ...
    def p_sample_loop(
        ...
        """
        assert not (
            recon_guidance and cond_fn_with_grad
        ), "Can use either recon guidance or classifier guidance, not both"
        final = None
        ...
        if return_with_prefix and "prefix" in model_kwargs["y"].keys():
            final["sample"] = torch.cat(
                [model_kwargs["y"]["prefix"], final["sample"]], dim=-1
            )
    ...
    def p_sample_loop_progressive(
        ...
        if init_image is not None:
            ...
            img = self.q_sample(init_image, my_t, img, model_kwargs)
    ...
    def ddim_sample_loop_progressive(
        ...
        # Orig code - not relevant for our usecase:
        # if init_image is not None:
        #     my_t = th.ones([shape[0]], device=device, dtype=th.long) * indices[0]
        ...
        # Our code:
        if init_image is not None:
            assert skip_timesteps == 0  # for ddim reconstruction
            img = init_image
        ...
        for i in indices:
            ...
            with GradEnable(
                enable_grad
            ):  # TODO(Roey): pass the grad enable bool as external argument
                sample_fn = (
    ...
    def training_losses(
        ...
        if "keyframe_mask" in model_kwargs["y"].keys():
            mask = torch.logical_and(
                mask, ~model_kwargs["y"]["keyframe_mask"]
            )  # exclude keyframes from loss

        # if "loss_mask" in model_kwargs['y'].keys():
        #     mask = torch.logical_and(mask, ~model_kwargs['y']['loss_mask'])
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if "loss_mask" in model_kwargs["y"]:

                def masked_l2_full(a, b, mask, loss_fn, eps=1e-8):
                    """Entry-wise L2 with a full-shape mask."""
                    loss = loss_fn(a, b)  # element-wise error
                    masked_loss = loss * mask.float()  # zero-out unwanted entries
                    num_valid = mask.float().sum()  # correct count of kept elements
                    return masked_loss.sum() / (num_valid + eps)

                loss_mask = model_kwargs["y"]["loss_mask"]  # e.g. (bs, J, 1, T)
                # base frame-level mask: (bs, 1, 1, T)
                frame_mask = mask.bool()
                # feature-level mask: make bool, broadcast to match target
                feat_mask = loss_mask.bool().expand_as(target)  # (bs, J, Jdim, T)
                # merge them & guarantee full shape
                full_mask = (frame_mask & feat_mask).expand_as(
                    target
                )  # (bs, J, Jdim, T)
                terms["rot_mse"] = masked_l2_full(
                    target, model_output, full_mask, lambda x, y: (x - y) ** 2
                )
            else:
