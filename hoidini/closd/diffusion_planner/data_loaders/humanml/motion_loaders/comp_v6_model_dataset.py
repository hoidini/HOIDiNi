...
import pickle
import numpy as np
from hoidini.closd.diffusion_planner.data_loaders.humanml.networks.modules import (
    TextEncoderBiGRU,
    TextDecoder,
    AttLayer,
    MovementConvEncoder,
...
from hoidini.closd.diffusion_planner.data_loaders.humanml.networks.trainers import (
    CompTrainerV6,
)
...
from hoidini.closd.diffusion_planner.data_loaders.humanml.utils.metrics import (
    calculate_skating_ratio,
    calculate_mean_penetration,
    calculate_penetration,
    calculate_floating,
    calculate_foot_sliding,
)
from hoidini.closd.diffusion_planner.data_loaders.humanml.utils.utils import (
    sample_to_motion,
)
from hoidini.closd.diffusion_planner.utils.sampler_util import AutoRegressiveSampler
from hoidini.closd.diffusion_planner.utils.loss_util import masked_goal_l2

# from inference import AutoRegressiveDNOSampler  #
...
class CompV6GeneratedDataset(Dataset):
    ...
    def __init__(self, opt, dataset, w_vectorizer, mm_num_samples, mm_num_repeats):
        ...
        text_enc, seq_pri, seq_dec, att_layer, mov_enc, mov_dec, len_estimator = (
            build_models(opt)
        )
...
class CompMDMGeneratedDataset(Dataset):
    def __init__(
        ...
        if self.args.autoregressive:
            if self.args.dataset == "smplrifke":
                sample_cls = AutoRegressiveDNOSampler(args, sample_fn)
        ...
        if self.hml_type is not None:
            ...
            self.used_mean = _t(
                np.load(pjoin(self.dataset.opt.data_root, f"Mean_{self.hml_type}.npy"))
            )
            self.used_std = _t(
                np.load(pjoin(self.dataset.opt.data_root, f"Std_{self.hml_type}.npy"))
            )
        ...
        with torch.no_grad():
            for i, (motion, model_kwargs) in tqdm(
                enumerate(dataloader),
                desc="Eval generate motions",
                ...
                if args.spatial_condition is not None:
                    if args.spatial_condition == "traj":
                        model_kwargs["y"]["condition_mask"] = torch.tensor(
                            humanml_utils.HML_ROOT_HORIZONTAL_MASK[None, :, None, None],
                            device=dist_util.dev(),
                        )
                        init_image = torch.tensor(
                            motion * model_kwargs["y"]["condition_mask"],
                            device=dist_util.dev(),
                        )
                ...
                if self.args.multi_target_cond:
                    assert (
                        self.args.target_joint_names == "DIMP_FINAL"
                    ), "We dont support the rest of the configurations anymore"
                    ...
                    (
                        ...
                        model_kwargs["y"]["is_heading"],
                    ) = sample_goal(dataloader.batch_size, motion.device, "PURE_T2M")
                    model_kwargs["y"]["target_cond"] = get_target_location(
                        motion,
                        self.dataset.mean_gpu,
                        self.dataset.std_gpu,
                        torch.tensor([motion.shape[-1]] * motion.shape[0]),
                        self.dataset.t2m_dataset.opt.joints_num,
                ...
                for t in range(repeat_times):
                    ...
                    cur_metrics = self.physical_metrics(
                        sample, model_kwargs["y"]["lengths"]
                    )
                    self.aux_metrics.append(cur_metrics)
    ...
    def convert_global_root(self, sample):
        ...
        denormed_global[:, :3, :, :] -= denormed_global[
            :, :3, :, [0]
    ...
    def calc_target_metrics(self, pred_motion, cond):
        pred_location = get_target_location(
            pred_motion,
            self.dataset.mean_gpu,
            self.dataset.std_gpu,
    ...
    def physical_metrics(self, pred_motion, lengths):
        ...
        return {
            "skate_ratio": torch.tensor(skate_ratio),
            "mean_penetration": torch.tensor(mean_penetration),
            "penetration": torch.tensor(penetration),
            "floating": torch.tensor(floating),
            "skating": torch.tensor(skating),
        }  # TODO: Remove dependence on torch, use numpy instead
...
def undo_recover_root_rot_pos(data):
    from hoidini.closd.diffusion_planner.data_loaders.humanml.common.quaternion import (
        qrot,
    )
    ...
    rel_pos[:, :, :-1] = rel_pos[
        :, :, 1:
    ].clone()  # very last element of relative positions is lost and the first is not necessarily 0 # try setting it to 0 too
...
class CompMDMExternalDataset(CompMDMGeneratedDataset):
    ...
    def __init__(
        ...
        if args.external_results_file.endswith(".npy"):
            external_motions = np.load(
                args.external_results_file, allow_pickle=True
            ).item()
        else:
        ...
        if args.do_unique and "db_key" in external_motions:
            ...
            shuffled_idx = np.random.permutation(
                range(external_motions["motion"].shape[0])
            )
            ...
            assert all(
                [
                    len(external_motions[k]) == len(unique_idx)
                    for k in external_motions.keys()
                ]
            ), "Unique motion count mismatch"
        ...
        for i, motion in enumerate(external_motions["motion"]):
            ...
            non_zero_length[i] = (
                torch.nonzero(motion.sum(axis=1))[-1].item() + 1
                if len(torch.nonzero(motion.sum(axis=1))) > 0
                else 0
            )
            adjusted_length[i] = min(
                max_length, non_zero_length[i], external_motions["length"][i]
            )
        ...
        self.generated_motion = [
            {
                ...
                "cap_len": tokens.split("_").index("eos/OTHER") + 1,
            }
            for motion, length, caption, tokens in zip(
                external_motions["motion"],
                adjusted_length,
                external_motions["caption"],
                external_motions["tokens"],
            )
        ]
        ...
        if True:  # self.args.physical_metrics:
            ...
            all_motions = torch.cat(motions_list).permute(0, 2, 1)[
                :, :, None, :
            ]  # B x 263 x 1 x n_frames
            lengths_list = [
                np.array(int(gen_m["length"]))[None] for gen_m in self.generated_motion
            ]
