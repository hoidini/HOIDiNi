...
from hoidini.closd.diffusion_planner.data_loaders.humanml.motion_loaders.comp_v6_model_dataset import (
    CompMDMGeneratedDataset,
    CompMDMExternalDataset,
)
...
def get_mdm_loader(
    ...
    scale,
    hml_type,
):  # ASK: what is args doing here? Comparing to mdm codebase, shouldn'e be here
    opt = {
        "name": "test",  # FIXME
    }
    print("Generating %s ..." % opt["name"])
    ...
    if args.external_mode:
        dataset = CompMDMExternalDataset(
            args,
            model,
            diffusion,
            ground_truth_loader,
