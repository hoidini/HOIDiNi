...
class Skeleton(object):
    ...
    def inverse_kinematics_np(
        ...
        root_quat = qbetween_np(
            forward, target
        )  # angle from root to Z+ (= how much to rotate root such that it faces Z+)
        ...
        for chain in self._kinematic_tree:
            ...
            for j in range(len(chain) - 1):
                ...
                u = self._raw_offset_np[chain[j + 1]][np.newaxis, ...].repeat(
                    len(joints), axis=0
                )  # rest-pose bone direction for joint j in the chain
                # print(u.shape)
                # (batch, 3)
                v = (
                    joints[:, chain[j + 1]] - joints[:, chain[j]]
                )  # data bone direction for joint j+1 in the chain
                v = v / np.sqrt((v**2).sum(axis=-1))[:, np.newaxis]
                # print(u.shape, v.shape)
                rot_u_v = qbetween_np(
                    u, v
                )  # angle betweem rest-pose bone and data bone (bone is j to j+1)

                R_loc = qmul_np(
                    qinv_np(R), rot_u_v
                )  # bring angle to be local coordinate system, i.e., relative to the parent bone
