...
def calculate_mean_penetration(motions, mask=None):
    ...
    per_joint_penalty = penetration.amin(dim=1).sum() * -1.0
    per_joint_frame_penalty = penetration.amin(dim=1).amin(dim=-1).sum() * -1.0
    non_zero_count = torch.sum(penetration != 0, dtype=torch.float32)

    return (
        per_joint_frame_penalty.numpy() * 100.0
    )  # / non_zero_count # convert to centimeters
...
def calculate_penetration(motions, lengths):
    ...
    for i, motion_len in enumerate(lengths):
        penetration = (
            (lowest_heights[i, :motion_len] + tolerance).clamp_max(0) * 1000 * -1.0
        )
...
def calculate_foot_sliding(motions, lengths):
    ...
    for joints, motion_len in zip(motions, lengths):
        ...
        for t in range(motion_len - 1):
            ...
            if (
                joints[contact_idx, 1, t] <= margin
                and joints[contact_idx, 1, t + 1] <= margin
            ):
                offset = (
                    joints[contact_idx, [0, 2], t + 1] - joints[contact_idx, [0, 2], t]
                )  # horizontal distance
