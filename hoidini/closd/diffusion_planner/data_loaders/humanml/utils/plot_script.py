...
def plot_3d_motion(
    ...
    def add_traj(x_offset, z_offset, plane_height):
        ax.scatter3D(
            ...
            s=5,
            c=[
                "b" if i in gt_frames else "orange"
                for i in range(trajec[:, 0].shape[0])
            ],
        )
            ...
            for k in ["goal_cond", "heading_cond", "heading_pred_cond"]:
                if k in cond.keys():
                    ...
                    if "heading" in k and cond["is_heading"]:  # heading angle
                        if cond[
                            "is_heading"
                        ]:  # if heading is not tested, do not draw it
    ...
    elif dataset in ["smplrifke", "humanml"]:
    ...
    def update(index):
        ...
        used_colors = used_colors * (
            len(kinematic_tree) // len(used_colors) + 1
