...
from hoidini.closd.diffusion_planner.data_loaders.humanml.utils.word_vectorizer import (
    WordVectorizer,
)
from hoidini.closd.diffusion_planner.data_loaders.humanml.utils.get_opt import get_opt
...
#         return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, None  # last token is for key
...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        cache_path = os.path.join(
            ...
            "data",
            "humanml3d",
            self.opt.dataset_name + "_" + _split + _name + ".npy",
        )
        if opt.use_cache and os.path.exists(cache_path):
            ...
            raise NotImplementedError("This project uses only cached data.")
            for name in tqdm(id_list):
                try:
                    motion = np.load(pjoin(opt.motion_dir, name + ".npy"))
                    if (len(motion)) < min_motion_len or (len(motion) >= 200):
        ...
        # structures related to removal by text
        self.prompt_to_key = (
            {}
        )  # FIXME: this dict becomes redundant once we return keys in __getitem__
        for key, data in self.data_dict.items():
            for t in data["text"]:
                self.prompt_to_key[t["caption"]] = self.prompt_to_key.get(
                    t["caption"], []
                ) + [key]
        # print(f'caption exists twice? {any([len(keys) >  1 for keys in self.prompt_to_key.values()])}')
    ...
    def remove_motion(self, key):
        ...
        for t in self.data_dict[key]["text"]:
            if len(self.prompt_to_key[t["caption"]]) == 1:
                del self.prompt_to_key[t["caption"]]
            else:
                key_idx = self.prompt_to_key[t["caption"]].index(key)
                self.prompt_to_key[t["caption"]] = (
                    self.prompt_to_key[t["caption"]][:key_idx]
                    + self.prompt_to_key[t["caption"]][key_idx + 1 :]
                )
        del self.data_dict[key]
        self.name_list = self.name_list[:idx] + self.name_list[idx + 1 :]
        self.length_arr = np.concatenate(
            [self.length_arr[:idx], self.length_arr[idx + 1 :]]
        )

    def remove_by_text(
        self, text
    ):  # FIXME: this function becomes redundant once we return keys in __getitem__
        n_removed = 0
        for t in text:
            # t is not n self.prompt_to_key one of the following applies:
            #    (1) it was sampled twice by the loader and now we are at the second time, or
            #    (2) another text that co-exists with it in the same motion caused that motion to be removed
            if t in self.prompt_to_key:
                key = self.prompt_to_key[t][0]
...
#         # print(word_embeddings.shape, motion.shape)
#         # print(tokens)
#         return word_embeddings, caption, sent_len, src_motion, tgt_motion, m_length, None  # last token is for key
...
#         text_data = random.choice(text_list)
#         caption, tokens = text_data['caption'], text_data['tokens']
#         return None, None, caption, None, np.zeros([self.fixed_length, 1]), self.fixed_length, None, None  # last token is for key
...
class HumanML3D(data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        opt = get_opt(dataset_opt_path, device, hml_type)
        ...
        opt.data_root = pjoin(opt.cache_dir, "data", opt.data_root)
        ...
        opt.disable_offset_aug = (
            is_autoregressive and (opt.fixed_len > 0) and (mode == "eval")
        )  # for autoregressive evaluation, use the start of the motion and not something from the middle
        ...
        if mode == "gt":
            ...
            if hml_type is None:
                ...
                name = f"_{hml_type}"
            self.mean = np.load(pjoin(opt.data_root, f"Mean{name}.npy"))
            self.std = np.load(pjoin(opt.data_root, f"Std{name}.npy"))
    ...
    def remove_by_text(self, text):
        return self.t2m_dataset.remove_by_text(text)
