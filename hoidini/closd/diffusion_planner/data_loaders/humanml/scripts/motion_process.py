...
from hoidini.closd.diffusion_planner.data_loaders.humanml.common.skeleton import (
    Skeleton,
)
import numpy as np
import os
from hoidini.closd.diffusion_planner.data_loaders.humanml.common.quaternion import *
from hoidini.closd.diffusion_planner.data_loaders.humanml.utils.paramUtil import *
...
def recover_root_rot_pos(data, hml_type=None):
    if hml_type == "global":
        # global xyz positions representation
        raise ValueError(
            "Function only applicaple for None and global_root representations."
        )
...
def recover_root_rot_heading_ang(joints, hml_type=None):
    ...
    forward = torch.cross(
        torch.tensor([[[0], [1], [0]]], dtype=across.dtype, device=across.device),
        across,
        axis=1,
    )
...
def traj_global2vel(traj_positions, traj_rot):
    ...
    velocity[:, :, [0, 2]] = (
        traj_positions[:, 1:, :] - traj_positions[:, :-1, :]
    ).clone()
    #     print(r_rot.shape, velocity.shape)
    ...
    l_velocity = velocity[:, :, [0, 2]]
    #     print(r_velocity.shape, l_velocity.shape, root_y.shape)
    root_data = torch.cat([r_velocity, l_velocity], axis=-1).permute(0, 2, 1)[
        :, :, None
    ]
...
def get_target_location(
    ...
    extended_goal_joint_names = all_goal_joint_names + [
        "traj",
        "heading",
    ]  # todo: fix hardcoded indexing that assumes traj and heading are last
    ...
    )  #  n_samples x (n_target_joints+1) x 3 x n_frames
    ...
    joints_loc = torch.concat(
        [joints_loc, joints_loc[:, pelvis_loc : pelvis_loc + 1]], dim=1
    )  # concatenate the pelvis location to be used for traj

    # joint names to indices
    HML_JOINT_NAMES_w_traj = HML_JOINT_NAMES + ["traj"]
    for sample_idx in range(batch_size):
        req_joint_idx_in = [
            HML_JOINT_NAMES_w_traj.index(name)
            for name in target_joint_names[sample_idx]
        ]
        req_joint_idx_out = [
            extended_goal_joint_names.index(name)
            for name in target_joint_names[sample_idx]
        ]

        target_loc[sample_idx, req_joint_idx_out] = joints_loc[
            sample_idx, req_joint_idx_in
        ]  # assign joints loc to output tensor
...
def hml_to_abs_loc(motion, mean, std, joints_num):
    ...
    joints_loc = recover_from_ric(
        unnormed_motion, joints_num
    )  # FIXME - not supporting global hml_type
    joints_loc = joints_loc.view(-1, *joints_loc.shape[2:]).permute(
        0, 2, 3, 1
    )  # n_samples x n_joints x 3 x n_frames
    return joints_loc
...
def sample_goal(batch_size, device, force_joints=None):
    if force_joints is None:
        choices = np.array(
            ["None", "traj", "pelvis"] + HML_EE_JOINT_NAMES
        )  # todo: fix hardcoded 'pelvis' ('traj' is ok because it's our convention)
        ...
        names = np.array(
            [np.delete(name, np.argwhere(name == "None")) for name in names]
        )
        is_heading = torch.bernoulli(torch.ones(batch_size, device=device) * 0.5).to(
            bool
        )
    else:
