...
def extract_features_torch(
    ...
    positions = get_rifke_torch(
        positions, r_rot
    )  # local pos relative to root at (0,y,0); rotate positions such that root faces Z+
    ...
    )  # root_y is cur joint, velocity is between current joint to the next one
    ...
    local_vel = qrot(
        ...
        global_positions[:, 1:] - global_positions[:, :-1],
    )  # SR: why not simply use positions[:, 1:] - positions[:, :-1]?
    local_vel = local_vel.reshape(bs, n_frames - 1, -1)

    data = root_data
    data = torch.cat(
        [data, ric_data[:, :-1]], axis=-1
    )  # omit position at last frame. so each frame has its own position and velocity to next frame
...
def foot_detect_torch(positions, thres, fid_l, fid_r):
    velfactor, heightfactor = torch.tensor([thres, thres]).to(
        positions.device
    ), torch.tensor([3.0, 2.0]).to(positions.device)
...
def get_rifke_torch(positions, r_rot):
    ...
    positions[..., 0] -= positions[
        ..., 0:1, 0
    ].clone()  # must clone in the torch implementation, otherwise there is a weird glitch
    positions[..., 2] -= positions[
        ..., 0:1, 2
    ].clone()  # must clone in the torch implementation, otherwise there is a weird glitch
    """All pose face Z+"""
    positions = qrot(
        torch.repeat_interleave(r_rot[:, :, None], positions.shape[2], axis=2),
        positions,
    )
    return positions
...
def get_cont6d_params_torch(
    positions, n_raw_offsets, kinematic_chain, face_joint_indx, fix_ik_bug
):
    skel = Skeleton(n_raw_offsets, kinematic_chain, "cpu")
    # (seq_len, joints_num, 4)
    # TODO - translate to torch!
    ...
    quat_params = skel.inverse_kinematics_np(
        ...
        fix_bug=fix_ik_bug,
    )  # TODO - cannot smooth with multi-batch in axis 0
    quat_params = (
        torch.from_numpy(quat_params)
        .reshape(bs, n_frames, n_joints, -1)
        .float()
        .to(positions.device)
    )
    ...
    ).clone()  # root joint velocity
    #     print(r_rot.shape, velocity.shape)
    velocity = qrot(
        r_rot[:, 1:], velocity
    )  # rotate the root joint velocity according to the direction of the root joint
    """Root Angular Velocity"""
    # (seq_len - 1, 4)
