...
class ClearmlPlatform(TrainPlatform):
    def __init__(self, save_dir):
        ...
        self.task = Task.init(project_name="RL", task_name=name)
...
class WandBPlatform(TrainPlatform):
    ...
    def __init__(self, save_dir, config=None, *args, **kwargs):
        ...
        self.wandb.init(
            project=kwargs.get("project", "amass_smplrifke"),
            name=self.name,
    ...
    def report_media(self, title, series, iteration, local_path):
        ...
        self.wandb.log(
            {series: [self.wandb.Video(file, format="mp4", fps=20) for file in files]},
            step=iteration,
        )

    def report_args(self, args, name, allow_val_change=False):
        self.wandb.config.update(
            args, allow_val_change=allow_val_change
        )  # , allow_val_change=True)  # use allow_val_change ONLY if you want to change existing args (e.g., overwrite)
