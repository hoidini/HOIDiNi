from hoidini.closd.diffusion_planner.utils.loss_util import masked_l2, angle_l2
...
def traj_cond_fn(x_zero_pred, t, y, mean, std):
    ...
    heading_rot, traj_pred = hml_to_root_loc_rot(
        x_zero_pred, mean, std
    )  # [bs, 1, n_frames], # [bs, 2, n_frames]
    # # DEBUG - start
    # full_pred = traj_pred - traj_pred[..., [y['prefix'].shape[-1]]]
    # # DEBUG - end
    if "prefix" in y.keys():
        traj_pred = (
            traj_pred[..., y["prefix"].shape[-1] :]
            - traj_pred[..., [y["prefix"].shape[-1]]]
        )
        heading_rot = (
            heading_rot[..., y["prefix"].shape[-1] :]
            - heading_rot[..., [y["prefix"].shape[-1]]]
        )
    ...
    loss = torch.tensor(0.0, device=traj_pred.device, dtype=traj_pred.dtype)
    ...
    if "heading_cond" in y.keys():
        loss += masked_l2(
            heading_rot, y["heading_cond"], y["recon_mask"], loss_fn=angle_l2
        ).sum()
...
def hml_to_root_loc_rot(sample, mean, std):
    ...
    r_rot_ang = torch.atan2(
        r_rot_quat[..., 2], r_rot_quat[..., 0]
    )  # relative to Z+, always starts with 0 (i.e. dront facing to Z+)
