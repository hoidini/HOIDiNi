...
import os
import json
from hoidini.closd.diffusion_planner.data_loaders.humanml_utils import (
    HML_EE_JOINT_NAMES,
)
...
def add_model_options(parser):
    ...
    group.add_argument(
        "--pos_embed_max_len", default=5000, type=int, help="Pose embedding max length."
    )
    group.add_argument(
        "--use_ema", action="store_true", help="If True, will use EMA model averaging."
    )
    ...
    group.add_argument(
        "--target_enc_layers", default=1, type=int, help="Num target encoder layers"
    )
    ...
    group.add_argument(
        ...
        default=0,
        type=int,
        help="If larger than 0, will do prefix completion.",
    )
    group.add_argument(
        "--pred_len",
        default=0,
        type=int,
        help="If context_len larger than 0, will do prefix completion. If pred_len will not be specified - will use the same length as context_len",
    )
...
def add_data_options(parser):
    ...
    group.add_argument(
        "--dataset",
        default="humanml",
        choices=["humanml", "smplrifke", "kit", "humanact12", "uestc"],
        type=str,
    ...
    group.add_argument(
        "--features_string",
        default="hoi_body_hands",
        type=str,
        choices=["human", "hoi", "hoi_hands", "human_w_contact", "human_w_any_contact"],
    )
    group.add_argument(
        "--condition_input_feature",
        type=str,
        default=None,
        help="If provided, will use it as a condition input feature.",
    )
...
def add_training_options(parser):
    ...
    group.add_argument(
        ...
        default=0.0,
        type=float,
        help="Probability of zeroing the prefix.",
    )
    ...
    group.add_argument(
        "--avg_model_beta", default=0.9999, type=float, help="Average model beta."
    )
    group.add_argument("--adam_beta2", default=0.999, type=float, help="Adam beta2.")
    group.add_argument(
        "--sampling_mode",
        default="none",
        choices=["none", "goal", "traj", "heading", "heading_traj"],
        type=str,
        help="For classifier-guidance conditioning.",
    )  # Placeholder for gen_during_training
    group.add_argument(
        "--target_joint_names",
        default="DIMP_FINAL",
        type=str,
...
def add_sampling_options(parser):
    ...
    group.add_argument(
        "--recon_param", default=1e3, type=float, help="Reconstruction parameter."
    )
    group.add_argument(
        "--recon_step_start",
        default=-1,
        type=int,
        help="Highest step index to perform recon_guidance. Default -1 means from first step.",
    )
    group.add_argument(
        "--recon_step_stop",
        default=2,
        type=int,
        help="Lowest step index to perform recon_guidance. 0 means until the last step.",
    )
    group.add_argument(
        "--recon_frame_start",
        default=0,
        type=int,
        help="First frame index to perform recon_guidance.",
    )
    group.add_argument(
        "--recon_frame_stop",
        default=-1,
        type=int,
        help="Last frame index to perform recon_guidance. Default -1 means from last step.",
    )
    group.add_argument(
        "--sampling_mode",
        default="none",
    ...
    group.add_argument(
        "--cfg_type",
        default="none",
        choices=["text", "target", "text_target"],
        type=str,
        help="For classifier-guidance conditioning.",
    )
    group.add_argument(
...
def add_generate_options(parser):
    ...
    group.add_argument(
        ...
        choices=["data", "random"],
        type=str,
        help="Either use targets from the data or choose random targets.",
    )
...
def add_evaluation_options(parser):
    ...
    group.add_argument(
        "--do_unique",
        action="store_true",
        help="If true, select only one motion for each db key.",
    )
    group.add_argument(
        "--eval_name",
        default="",
        type=str,
        help="Optional for wandb. if empty will use the model name instead.",
    )
    group.add_argument(
        "--eval_mode",
        default="wo_mm",
    ...
    # group.add_argument("--dataset_data", default='hml3d', choices=['hml3d', 'grab', 'hml3d_grab'], type=str)


def get_cond_mode(args):
    if args.unconstrained:
        cond_mode = "no_cond"
    elif args.dataset in ["smplrifke", "kit", "humanml"]:
        cond_mode = "text"
    else:
        cond_mode = "action"
    return cond_mode
...
def train_args():
    ...
    group = parser.add_argument_group("smplrifke")
    # group.add_argument("--dataset_data", default='hml3d', choices=['hml3d', 'grab', 'hml3d_grab'], type=str)
    group.add_argument(
        "--data_load_lim",
        type=int,
        default=None,
        help="Limit the number of data loading iterations.",
    )
    group.add_argument(
        "--use_obj_encoder",
        type=bool,
        default=True,
        help="If true, will use object encoder.",
    )
    group.add_argument(
        "--use_bps_mlp",
        type=bool,
        default=True,
        help="If true, will use bps mlp encoder.",
    )
