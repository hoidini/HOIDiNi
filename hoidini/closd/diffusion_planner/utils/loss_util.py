...
def masked_goal_l2(pred_goal, ref_goal, cond, all_goal_joint_names):
    ...
    target_joint_idx = [
        [np.where(all_goal_joint_names_w_traj == j)[0][0] for j in sample_joints]
        for sample_joints in cond["target_joint_names"]
    ]
    ...
    loc_loss = masked_l2(
        pred_goal[:, :-1], ref_goal[:, :-1], loc_mask, entries_norm=False
    )

    heading_loss = masked_l2(
        pred_goal[:, -1:, :1],
        ref_goal[:, -1:, :1],
        cond["is_heading"].unsqueeze(1).unsqueeze(1),
        loss_fn=angle_l2,
        entries_norm=False,
    )
