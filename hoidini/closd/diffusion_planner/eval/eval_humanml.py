import os
from hoidini.closd.diffusion_planner.utils.parser_util import evaluation_parser
from hoidini.closd.diffusion_planner.utils.fixseed import fixseed
from datetime import datetime
from hoidini.closd.diffusion_planner.data_loaders.humanml.motion_loaders.model_motion_loaders import (
    get_mdm_loader,
)  # get_motion_loader
...
def evaluation(
    ...
    with open(log_file, "w") as f:
        ...
        for replication in range(replication_times):
            ...
            if hasattr(motion_loader.dataset, "aux_metrics"):
                aux_title = "AUX METRICS:"
                print(aux_title)
                print(aux_title, file=f, flush=True)
                aux_names = list(
                    motion_loader.dataset.aux_metrics[0].keys()
                )  # Ask: this line throws error: IndexError: list index out of range
                if replication == 0:
                    all_metrics.update({k: OrderedDict() for k in aux_names})
                for metric in aux_names:
                    metric_vals = np.array(
                        [
                            e[metric].cpu().numpy()
                            for e in motion_loader.dataset.aux_metrics
                        ]
                    )
                    assert len(eval_motion_loaders) == 1
                    key = list(eval_motion_loaders.keys())[0]
                    if key not in all_metrics[metric]:
                        all_metrics[metric][key] = [metric_vals.mean()]
                    else:
                        all_metrics[metric][key] += [metric_vals.mean()]
                    aux_str = f"---> [{motion_loader_name}] {metric}: {metric_vals.mean():.4f}"
                    print(aux_str)
                    print(aux_str, file=f, flush=True)
        ...
        if eval_platform is not None:
            for k, v in mean_dict.items():
                if k.startswith("R_precision"):
                    ...
                    eval_platform.report_scalar(
                        name=k, value=v, iteration=1, group_name="Eval"
                    )
...
if __name__ == "__main__":
    ...
    assert (
        args.model_path != ""
        or args.external_results_file != ""
        and not (args.model_path != "" and args.external_results_file != "")
    ), "either model_path or external_results_file args should be specified!"
    ...
    eval_platform.report_args(
        args, name="Args", allow_val_change=args.eval_mode == "debug"
    )
    ...
    split = "test"
    gt_ds_data = "humanml" if args.dataset == "smplrifke" else args.dataset
    ...
    gen_loader = get_dataset_loader(
        ...
        data_load_lim=args.eval_mode == "debug",
    ...
    evaluator_ds_name = "humanml" if args.dataset == "smplrifke" else args.dataset
    eval_wrapper = EvaluatorMDMWrapper(evaluator_ds_name, dist_util.dev())
    evaluation(
        eval_wrapper,
        gt_loader,
        eval_motion_loaders,
