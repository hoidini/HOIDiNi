...
def main():
    ...
    if out_path == "":
        out_path = os.path.join(
            ...
            "invert_{}_{}_{}_seed{}".format(name, niter, args.edit_mode, args.seed),
        )
        if args.text_condition != "":
            out_path += "_" + args.text_condition.replace(" ", "_").replace(".", "")
        if args.guidance_param != 1.0:
            out_path += f"_gscale{args.guidance_param}"
    ...
    # reverse samples with the original text
    ...
    for rep_i in range(args.num_repetitions):
        ...
        # invert sample to noise
        print("Inverting input samples to noise.")
        ...
        print("Reconstructing input samples.")
        sample = sample_fn(
            model,
    ...
    for sample_i in range(args.num_samples):
        ...
        for rep_i in range(args.num_repetitions):
            ...
            caption = "DDIM Reconstruction"
            if args.text_condition != "":
                caption = "DDIM Edit: {}".format(args.text_condition)
