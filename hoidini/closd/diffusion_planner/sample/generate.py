...
from hoidini.closd.diffusion_planner.utils.parser_util import generate_args
...
from hoidini.closd.diffusion_planner.utils.sampler_util import (
...
import hoidini.closd.diffusion_planner.data_loaders.humanml.utils.paramUtil as paramUtil
from hoidini.closd.diffusion_planner.data_loaders.humanml.utils.plot_script import (
    plot_3d_motion,
)
import shutil
from hoidini.closd.diffusion_planner.data_loaders.tensors import collate
from moviepy.editor import clips_array
from hoidini.closd.diffusion_planner.utils import cond_util
from hoidini.closd.diffusion_planner.data_loaders import humanml_utils

# from amasstools.smplrifke_feats__obsolete import smplrifkefeats_to_smpldata
from datasets.dataset_smplrifke import SmplRifkeDataset
from general_utils import zup_to_yup
...
def main(args=None):
    if args is None:
        ...
        # import json
        # with open('/home/dcor/roeyron/trumans_utils/src/generate_args_working.json', 'w') as fw:
    ...
    max_frames = 196 if args.dataset in ["smplrifke", "kit", "humanml"] else 60
    fps = 12.5 if args.dataset == "kit" else 20
    n_frames = (
        min(max_frames, int(args.motion_length * fps))
        if args.motion_length is not None
        else max_frames
    )
    cfg_type = getattr(args, "cfg_type", "text")
    ...
    if out_path == "":
        ...
        if args.multi_target_cond:
            ...
            if args.target_joint_names is not None:
                out_path += "_joints-{}".format(
                    args.target_joint_names.replace(",", "-")
                )
    ...
    if is_using_data:
        ...
        if "prefix" in model_kwargs["y"].keys():
            input_motion_w_prefix = torch.concat(
                [model_kwargs["y"]["prefix"].to(dist_util.dev()), input_motion], dim=3
            )
    else:
        collate_args = [
    ...
    if args.spatial_condition is not None:
        if args.spatial_condition == "traj":
            ...
            init_image = torch.tensor(
                input_motion * model_kwargs["y"]["condition_mask"],
            ...
            model_kwargs["y"][
                ...
                init_image.clone()
            )  # FIXME - GUY - Looks like a bug, why using the traj from data if we override it with a stright line?
        else:
            raise ValueError(
                f"unsupported spatial_condition [{args.spatial_condition}]"
            )
    ...
    if args.sampling_mode == "goal":
        target_joint_names, is_heading = sample_goal(
            args.num_samples, dist_util.dev(), args.target_joint_names
        )
        ...
        if args.target_joint_source == "random":
            ...
            for sample_i, joint_list in enumerate(target_joint_names):
                if is_heading[sample_i]:
                    target_loc[sample_i, all_joint_names.index("heading"), 0] = (
                        2
                        * torch.pi
                        * torch.rand(
                            [1], dtype=target_loc.dtype, device=target_loc.device
                        )
                        - torch.pi
                    )
                for j in joint_list:
                    mid_point = torch.zeros(
                        [3], dtype=target_loc.dtype, device=target_loc.device
                    )
                    ...
                    radius[1] = 0.3
                    target_loc[sample_i, all_joint_names.index(j)] = (
                        mid_point + 2 * radius * torch.rand_like(mid_point) - radius
                    )
    ...
    for rep_i in range(args.num_repetitions):
        ...
        if args.dataset == "smplrifke":
            ds: SmplRifkeDataset = data.dataset
            normalizer: Normalizer = ds.normalizer
            normalizer.eps = 1e-9
            sample_raw = sample  # (batch, n_features, 1, seq_len)
            sample_raw = (
                sample_raw.cpu().permute(0, 2, 3, 1).float().squeeze()
            )  # (batch, seq_len, n_features)
            sample_normalized = normalizer.denormalize(
                sample_raw
            )  # (batch, seq_len, n_features)
            smpl_data_lst = [
                smplrifkefeats_to_smpldata(s, ds.n_joints) for s in sample_normalized
            ]  # List[(batch, seq_len, n_joints, 3)]
            joints_batch = torch.stack(
                [smpl_data["joints"] for smpl_data in smpl_data_lst]
            )  # (batch, seq_len, n_joints, 3)
            joints_batch = zup_to_yup(joints_batch)
            sample = joints_batch.permute(0, 2, 3, 1)  # (batch, n_joints, 3, seq_len)
        else:
    ...
    if args.dataset == "smplrifke":
        if args.dataset_data == "hml3d":
            skeleton = paramUtil.smpl_24_kinematic_chain
        elif args.dataset_data in ["hml3d_grab", "grab"]:
            skeleton = paramUtil.smplx_52_kinematic_chain
    else:
        skeleton = (
    ...
    for sample_i in range(args.num_samples):
        ...
        for rep_i in range(args.num_repetitions):
            ...
            motion = all_motions[rep_i * args.batch_size + sample_i].transpose(
                2, 0, 1
            )  # [:length]
            ...
            if "target_joint_names" in model_kwargs["y"].keys():
                cond.update(
                    {"joint_names": model_kwargs["y"]["target_joint_names"][sample_i]}
                )
...
def load_dataset(args, max_frames, n_frames):
    data = get_dataset_loader(
        ...
        device=dist_util.dev(),
        data_load_lim=(
            args.limit_data_loading if "limit_data_loading" in args else False
        ),
