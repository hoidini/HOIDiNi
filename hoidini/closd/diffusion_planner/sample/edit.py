...
import numpy as np
import torch
from hoidini.closd.diffusion_planner.utils.parser_util import edit_args
...
def main():
    ...
    if out_path == "":
        ...
        if args.edit_mode == "in_between":
            out_path += f"_{args.prefix_end}-{args.suffix_start}"
        if args.apply_cond:
            out_path += "_applyCond"
    ...
    elif args.edit_mode == "trajectory":
        model_kwargs["y"]["inpainting_mask"] = torch.tensor(
            humanml_utils.HML_TRAJ_MASK, dtype=torch.bool, device=input_motions.device
        )  # True is lower body data
        model_kwargs["y"]["inpainting_mask"] = (
            model_kwargs["y"]["inpainting_mask"]
            .unsqueeze(0)
    ...
    if args.apply_cond:
        model_kwargs["y"]["clean_motion"] = copy.deepcopy(
            model_kwargs["y"]["inpainted_motion"]
        )
        model_kwargs["y"]["keyframe_mask"] = copy.deepcopy(
            model_kwargs["y"]["inpainting_mask"][:, [0]]
        )
    ...
    animations = np.empty(
        shape=(args.num_samples, args.num_repetitions + 1), dtype=object
    )
    ...
    for sample_i in range(args.num_samples):
        ...
        animations[sample_i, 0] = plot_3d_motion(
            animation_save_path,
            skeleton,
            motion,
            title=caption,
        ...
        for rep_i in range(args.num_repetitions):
            ...
            animations[sample_i, rep_i + 1] = plot_3d_motion(
                animation_save_path,
                skeleton,
                motion,
                title=caption,
                dataset=args.dataset,
                fps=fps,
                vis_mode=args.edit_mode,
                gt_frames=gt_frames_per_sample.get(sample_i, []),
            )
            # Credit for visualization: https://github.com/EricGuo5513/text-to-motion

    save_multiple_samples(
        out_path, {"all": all_file_template}, animations, fps, max_frames
    )
