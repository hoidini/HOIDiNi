...
from hoidini.closd.diffusion_planner.data_loaders.humanml.scripts.motion_process_torch import (
...
def angle_to_2d_rotmat(angle):
    ...
    return torch.cat(
        [
            torch.cat([torch.cos(_angle), -torch.sin(_angle)], dim=-1)[:, :, None],
            torch.cat([torch.sin(_angle), torch.cos(_angle)], dim=-1)[:, :, None],
        ],
        dim=-1,
    )
...
def is_point_in_rectangle_vectorized(points, rect_corners):
    ...
    rect_area = area_vectorized(x1, y1, x2, y2, x3, y3) + area_vectorized(
        x1, y1, x4, y4, x3, y3
    )
...
class RepresentationHandler:
    ...
    def __init__(self, mean, std, init_humanoid_root_pose, time_prints=True):
        ...
        self.smpl2sim_rot_mat = torch.matmul(
            self.to_isaac_mat, self.to_isaac_mat
        )  # weird way to do it
        # self.smpl2sim_rot_mat = self.to_isaac_mat.dot(self.to_isaac_mat)  # weird way to do it
        self.y180_rot = (
            torch.from_numpy(
                sRot.from_euler(
                    "xyz", np.array([0, -np.pi, 0]), degrees=False
                ).as_matrix()
            )
            .cuda()
            .float()
    ...
    def hml_to_pose(self, pose, recon_data, sim_at_hml_idx):
        ...
        aligned_hml_xyz = self.align_to_recon_data(
            zeroed_hml_pose, recon_data, is_inverse=True
        )
        ...
        if self.time_prints:
            print(
                "=== hml_to_pose for [{}] envs took [{:.2f}] sec".format(
                    pose.shape[0], time.time() - start_time
                )
            )
    ...
    def pose_to_hml(self, pose, aux_ponts=None, fix_ik_bug=False):
        ...
        hml_pose, recon_data = self.orig_pose_to_hml(
            orig_pose, fix_ik_bug=fix_ik_bug
        )  # add back bs placeholder
        ...
        if aux_ponts is not None:
            ...
            assert (
                translated_aux_ponts
                - self.align_to_recon_data_old(aux_ponts_hml, recon_data)
                < 1e-6
            ).all()
        else:
            translated_aux_ponts = None
        # print(recon_data)
        normed_hml_pose = (hml_pose - self.mean.to(hml_pose.device)) / self.std.to(
            hml_pose.device
        )
        if self.time_prints:
            print(
                "=== pose_to_hml for [{}] envs took [{:.2f}] sec".format(
                    pose.shape[0], time.time() - start_time
                )
            )
    ...
    def align_to_recon_data_old(self, aux_ponts, recon_data):
        ...
        aux_ponts = qrot(
            torch.repeat_interleave(
                recon_data["r_rot"][:, None], aux_ponts.shape[1], axis=1
            ),
            aux_ponts,
        )
    ...
    def inverse_align_to_recon_data(self, points, recon_data):
        ...
        for _ in range(
            points.dim() - 2
        ):  # expand rot according to #frames and #joints (if the latter exists)
            rot = rot.unsqueeze(1)
            r_pos = r_pos.unsqueeze(1)
    ...
    def pose_to_hml_xyz(self, pose, aux_ponts=None):
        ...
        smpl_pose, translated_aux_ponts = self.xyz_to_smpl(
            xyz_pose, translated_aux_ponts
        )
    ...
    def xyz_to_smpl(self, xyz_pose, aux_ponts=None):
        ...
        smpl_pose = torch.matmul(
            smpl_pose, self.y180_rot.T.to(smpl_pose.device)
        )  # Guy's addition - to avoid the initial 180 deg turn at the begining of the sim
        smpl_pose = torch.matmul(
            smpl_pose, self.smpl2sim_rot_mat.T.to(smpl_pose.device)
        )
        if aux_ponts is not None:
            translated_aux_ponts = aux_ponts.clone()
            translated_aux_ponts[..., 1] -= self.offset
            translated_aux_ponts = torch.matmul(
                translated_aux_ponts, self.y180_rot.T
            )  # Guy's addition - to avoid the initial 180 deg turn at the begining of the sim
            translated_aux_ponts = torch.matmul(
                translated_aux_ponts, self.smpl2sim_rot_mat.T
            )
        else:
            translated_aux_ponts = None
    ...
    def xyz_to_full_smpl(self, pred_xyz):
        ...
        left = (
            mdm_jts[..., -2, :]
            + direction / torch.linalg.norm(direction, dim=-1, keepdim=True) * hand_len
        )
        ...
        mdm_jts_smpl_24 = torch.cat(
            [mdm_jts, left[..., None, :], right[..., None, :]], axis=-2
        )
    ...
    def smpl_to_sim(self, smpl_xyz):
        ...
        sim_xyz = torch.matmul(
            sim_xyz, self.y180_rot
        )  # Guy's additin - to avoid the initial 180 deg turn at the begining of the sim
    ...
    def coordinate_poses(self, pred_poses, cur_pose):
        ...
        pred_ee_vecs = (
            first_pred_pose[:, self.smpl_end_effectors_idx] - first_pred_pose[:, [0]]
        )
        ...
        avg_euler = torch.cat(
            [torch.zeros_like(avg_ang), avg_ang, torch.zeros_like(avg_ang)], dim=-1
        )  # [n_env, 3]
        avg_quat = transforms.matrix_to_quaternion(
            transforms.euler_angles_to_matrix(avg_euler, convention="XYZ")
        )
        pred_poses = qrot(
            avg_quat[:, None, None]
            .repeat_interleave(pred_poses.shape[1], dim=1)
            .repeat_interleave(pred_poses.shape[2], dim=2),
            pred_poses,
        )
    ...
    def fps_convert(self, mdm_jts, src_fps, trg_fps, interp_mode="bicubic"):
        ...
        if interp_mode == "linear":
            ...
            return (
                interp_out.squeeze(-1).permute(0, 2, 1).reshape(bs, -1, n_joints, n_dim)
            )
    ...
    def ref_pose_to_sim_pose(self, ref_pose):
        ...
        sim_pose[..., 1] *= -1.0
        sim_pose = sim_pose[..., [0, 2, 1]][
            ..., smpl_2_mujoco, :
        ]  # FIXME: double check whether smlpl_2_mujoco is required
        return sim_pose
