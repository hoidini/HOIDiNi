...
def link_all_checkpoints(dependencies_path):
    link_checkpoints(
        os.path.join(dependencies_path, "checkpoints", "dip"),
        os.path.join("closd", "diffusion_planner", "save"),
    )  # DiP checkpoints
    link_checkpoints(
        os.path.join(dependencies_path, "evaluation"),
        os.path.join("closd", "diffusion_planner", "saved_motions"),
    )  # DiP checkpoints
    link_checkpoints(
        os.path.join(dependencies_path, "checkpoints", "closd"),
        os.path.join("output", "CLoSD"),
    )  # CLoSD checkpoints
