...
from hoidini.closd.utils.smpllib.khrylib.models.tcn import TemporalConvNet
from hoidini.closd.utils.smpllib.khrylib.models.rnn import RNN
from hoidini.closd.utils.smpllib.khrylib.utils.torch import *
...
class VideoStateNet(nn.Module):
    ...
    def initialize(self, x):
        if self.mode == "test":
            self.v_out = self.forward_v_net(x.unsqueeze(1)).squeeze(1)[
            ...
            self.cnn_feat_ctx = np.zeros(
                (max_episode_len + 2 * self.v_margin, num_episode, self.cnn_feat_dim)
            )
            for i in range(num_episode):
                exp_ind, start_ind = v_metas[i, :]
                self.cnn_feat_ctx[:, i, :] = cnn_feat[exp_ind][
                    start_ind
                    - self.v_margin : start_ind
                    + max_episode_len
                    + self.v_margin
                ]
            self.cnn_feat_ctx = tensor(self.cnn_feat_ctx, dtype=dtype, device=device)
            self.scatter_indices = LongTensor(
                np.tile(self.indices[:, None], (1, self.cnn_feat_dim))
            ).to(device)
            self.gather_indices = LongTensor(
                np.tile(self.indices[:, None], (1, self.v_hdim))
            ).to(device)
    ...
    def forward(self, x):
        if self.mode == "test":
            ...
            v_ctx = self.forward_v_net(self.cnn_feat_ctx)[
                self.v_margin : -self.v_margin
            ]
...
if __name__ == "__main__":
    ...
    cnn_feat_dict, _ = pickle.load(
        open(os.path.expanduser("~/datasets/egopose/features/cnn_feat_0111.p"), "rb")
    )
