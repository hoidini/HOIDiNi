    ...
    def initialize(self, x):
        if self.mode == "test":
            ...
            for i in range(num_episode):
                ...
                self.cnn_feat_ctx[: self.v_margin, i, :] = cnn_feat[exp_ind][
                    start_ind - self.v_margin : start_ind
                ]
            self.cnn_feat_ctx = tensor(self.cnn_feat_ctx, dtype=dtype, device=device)
            self.s_scatter_indices = LongTensor(
                np.tile(self.indices[:, None], (1, self.state_dim))
            ).to(device)
            self.s_gather_indices = LongTensor(
                np.tile(self.indices[:, None], (1, self.s_hdim))
            ).to(device)
    ...
    def forward(self, x):
        if self.mode == "test":
            ...
            if self.s_net_type == "lstm":
                ...
                s_ctx = (
                    s_ctx.view(-1, self.max_episode_len, self.state_dim)
                    .transpose(0, 1)
                    .contiguous()
                )
                s_ctx = (
                    self.s_net(s_ctx).transpose(0, 1).contiguous().view(-1, self.s_hdim)
                )
