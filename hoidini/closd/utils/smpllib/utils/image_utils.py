...
def get_chunk_with_overlap(num_frames, window_size=80, overlap=10, return_idxes=False):
    ...
    chunk_supp = np.linspace(
        num_frames - window_size, num_frames - 1, num=window_size
    ).astype(int)
    chunk_idxes = np.concatenate((chunk_idxes, chunk_supp[None,]))
    last_chunk = (
        chunk_idxes[-1][:step][-1] - chunk_idxes[-2][:step][-1] + int(overlap / 2)
    )
    chunck_selects = get_chunk_selects(
        chunk_idxes, last_chunk, window_size=window_size, overlap=overlap
    )
    if return_idxes:
