...
def gaussian_filter1d_torch(sigma, order=0, truncate=4.0):
    # [1, 1, kernel_size]
    return torch.from_numpy(gaussian_filter1d(sigma, order, truncate).copy()).float()[
        None, None, :
    ]
...
def apply_filter1d_torch(input, kernel):
    ...
    if mid_idx - radius > 0:
        truncated_kernel = kernel[
            :, :, mid_idx - radius : mid_idx + radius + 1
        ].clone()  # [1, 1, seqlen+1]
    else:
        truncated_kernel = kernel.clone()
    truncated_kernel /= truncated_kernel.sum()  # normalize
    padded_input = torch.nn.functional.pad(
        input.permute(0, 2, 1).reshape(bs * ch, 1, seqlen),
        pad=(radius, radius),
        mode="reflect",
    )  # [bs, ch, seqlen*2]
    filtered_result = (
        torch.conv1d(padded_input, truncated_kernel)
        .reshape(bs, ch, -1)
        .permute(0, 2, 1)
    )  # -1 = len() - len(truncated_kernel) + 1
...
def apply_one_side_filter1d_torch(input, kernel):
    ...
    mid_idx = kernel.shape[-1] // 2
    truncated_kernel = kernel[
        :, :, max(0, mid_idx - radius + 1) : mid_idx + 1
    ].clone()  # [1, 1, radius]
    truncated_kernel /= truncated_kernel.sum()  # normalize
    truncated_input = input.permute(0, 2, 1).reshape(bs * ch, 1, seqlen)[
        :, :, -truncated_kernel.shape[-1] :
    ]  # [bs*ch, 1, radius]
    filtered_last_entry = torch.sum(
        truncated_input * truncated_kernel, dim=-1
    )  # [bs*ch, 1]  # dot product
